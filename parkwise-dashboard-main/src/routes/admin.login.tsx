...
function AdminLogin() {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Use window.location.href to force a hard page reload
      // This ensures all state is fresh and prevents token caching issues
  ...
  return (
    <div className="min-h-screen flex flex-col lg:flex-row bg-background overflow-hidden">
      ...
      <motion.div
        ...
        className="lg:hidden relative gradient-primary px-6 pt-8 pb-16 text-white z-20 shrink-0 shadow-lg"
      >
        ...
        >
          <h2 className="text-3xl font-bold leading-tight">Admin Control Center</h2>
          <p className="mt-2 text-white/80 text-sm">Manage sites, live occupancy & revenue.</p>
        </motion.div>
      </motion.div>
      ...
      <motion.div
        ...
        className="hidden lg:flex relative gradient-primary p-12 flex-col justify-between text-white z-20 shrink-0 shadow-2xl"
      >
      ...
      >
        <form onSubmit={submit} className="w-full max-w-md space-y-5 bg-card p-8 rounded-3xl shadow-card border border-border">
          <div className="mb-2">
            ...
            <h1 className="text-3xl font-bold text-center lg:text-left">Admin Sign In</h1>
            <p className="text-muted-foreground mt-2 text-sm text-center lg:text-left">Enter your credentials to access the admin dashboard.</p>
          </div>
